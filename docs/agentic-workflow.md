...
The planner asks only for facts that change the output: product/offer, campaign objective, audience, placement/platform, and CTA. It optionally collects brand assets, brand constraints, mandatory legal copy, and exact in-image copy. It must not ask a menu question such as “prompt or image?”
...
Use the orchestrator as the user-facing agent. The planner and executor are constrained role configurations for separate reviews or future agent-to-agent delegation. Disable sandbox, generated UI, file downloads, dynamic subagents, and large-tool-response offloading for all three. Preload the listed tools.
